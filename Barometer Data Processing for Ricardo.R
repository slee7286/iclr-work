                            ...
                            quant_step = c(0.01, 0.06))  # [K, Pa] if used
{
  ...
  for (k in seq_len(n)) {
    ...
    b_drift <- phi * b_prev + omega
    b_prev  <- b_drift                   # persist
    
    # Composite bias
    b_comp <- b_const + b_drift
    
    # Measurement noise
    noise <- noise_sd * rnorm(2)
...
telem_data1 <- telem_data1 %>% # Calculates moving average
  mutate(
    baro_temp = stats::filter(baro_temp, rep(1/100,100), sides=1),
    baro_press = stats::filter(baro_press, rep(1/100,100), sides=1)
  )
...
telem_data1[c("baro_temp")] <- telem_data1[c("baro_temp")] - temp_avg - temp_bias

telem_data1[c("baro_press")] <- telem_data1[c("baro_press")] - pressure_avg - pressure_bias
...
q_temp <- var(telem_data1$eps_temp, na.rm = T)
q_press <- var(telem_data1$eps_press, na.rm = T)
...
true_data <- data.frame(
  true_temp = rep(288.15, 1000),
  true_press = rep(99438, 1000)
)

sensor_out <- simulate_sensor(true_data$true_temp,
                              true_data$true_press,
                              dt       = dt,
                              b_const  = b_const,
                              tau      = tau,
                              q        = q,
